    ...
    public class ModLifecycle
    {
        ...
        private void PatchByHarmony()
        {
            try
            {
                ...
                // NOTE: KitsuneCommand no longer ships a paint-index widener.
                // The 255-paint network limit is now broken entirely by the
                // standalone KitsunePaintUnlocked mod (0_PaintUnlocked), which
                // owns the NetPackageSetBlockTexture Setup/write/read/ProcessPackage
                // patches. KC applying its own copy on top double-patched the
                // same methods — KC's stale 21-byte-encoding ReadPrefix threw
                // an NRE in the net deserializer and kicked any client who
                // painted. Removed entirely; KC is out of the paint-networking
                // business.
            }
            catch (Exception ex)
            {
